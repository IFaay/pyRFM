...
def func_f(p: torch.Tensor, t: torch.Tensor, normal: torch.Tensor, mean_curvature: torch.Tensor) -> torch.Tensor:
    ...
    Uses: Δ_S u = Δ u - n^T (Hess u) n - 2H * (∇u · n), f = ∂ₜ u - Δ_S u
    ...
    dt_u = torch.cos(p[:, [0]] + torch.sin(t)) * torch.exp(torch.cos(p[:, [1]] - p[:, [2]])) * torch.cos(t)
    return (dt_u - lb).detach()  # f = ∂ₜ u - Δ_S u
...
if __name__ == '__main__':
    ...
    for shape in ["ellipsoid", "torus", "genus2", "cheese"]:
        ...
        for dt in [1e-1, 5e-2, 2e-2, 1e-2, 5e-3]:
            t0 = time.time()
            t = 0.0
