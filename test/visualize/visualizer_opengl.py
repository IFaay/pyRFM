...
class RFMVisualizer(VisualizerBase):
    def __init__(self, model: RFMBase, resolution=(1920, 1080), component_idx=0):
        ...
        self.centers = model.centers.reshape(-1, 2).cpu().numpy().astype('f4')
        self.radii = model.radii.reshape(-1, 2).cpu().numpy().astype('f4')
        self.weights = np.array([submodel.weights.cpu().numpy() for submodel in model.submodels.flat_data],
                                dtype=np.float64)
        self.biases = np.array([submodel.biases.cpu().numpy() for submodel in model.submodels.flat_data],
                               dtype=np.float64)
        self.outputW = model.W[:, component_idx].cpu().numpy()
...
class RFMVisualizer2D(RFMVisualizer):
    def __init__(self, model: RFMBase, resolution=(1920, 1080), component_idx=0, pou_type=0, act_type=0):
    ...
    def plot(self):
        ...
        self.ax.imshow(np_img, extent=[self.bbox_min[0], self.bbox_max[0],
        ...
        self.ax.set_title('2D SDF Field (Viridis)')
        # ax.grid(True);
        plt.tight_layout()
    ...
    def _init_gl(self):
        ...
        def double2float2(x: np.ndarray) -> np.ndarray:
            hi = x.astype(np.float32)
            lo = (x - hi.astype(np.float64)).astype(np.float32)
            return np.stack((hi, lo), axis=-1)

        # Texture 2: weights (vec2), layout = (row = subfeature, col = hidden neuron)
        w_tex_data = self.weights.transpose(0, 2, 1)  # shape (n_sub, n_hidden, 2)
        w_tex_data = double2float2(w_tex_data).reshape(self.n_subdomain, self.n_hidden, 4)
        self.weights_tex = self.ctx.texture((self.n_hidden, self.n_subdomain), 4, data=w_tex_data.tobytes(), dtype='f4')
        ...
        b_tex_data = double2float2(self.biases)
        self.biases_tex = self.ctx.texture((self.n_hidden, self.n_subdomain), 2,
                                           data=b_tex_data.tobytes(),
                                           dtype='f4')
        ...
        outw = self.outputW.reshape(self.n_subdomain, self.n_hidden)
        outw = double2float2(outw)
        self.outputW_tex = self.ctx.texture((self.n_hidden, self.n_subdomain), 2, data=outw.tobytes(), dtype='f4')
    ...
    def _build_fragment_shader(self):
        ...
        uniform float vmax;
                ...
                // --- constants ---
        const double LN2_HI = 0.6931471805599453;         // High part of ln(2)
        const double LN2_LO = 1.9082149292705877e-10;     // Low part for extra precision
        const double INV_LN2 = 1.4426950408889634;        // 1 / ln(2)
        const double MAX_EXP = 709.782712893384;          // max valid input for exp()
        const double MIN_EXP = -745.1332191019411;        // min valid input for exp()
        
        // --- polynomial coefficients for exp(r), r in [-0.5*ln2, 0.5*ln2] ---
        // Approximates exp(r) = 1 + r + r^2/2! + r^3/3! + ... up to 10 terms
        double exp_poly(double r) {{
            ...
            double r9 = r6 * r3;
            double r10 = r5 * r5;
            return (1.0 + r + r2 * 0.5 + r3 * (1.0 / 6.0) + r4 * (1.0 / 24.0)
                    + r5 * (1.0 / 120.0) + r6 * (1.0 / 720.0) + r7 * (1.0 / 5040.0)
                    + r8 * (1.0 / 40320.0) + r9 * (1.0 / 362880.0) + r10 * (1.0 / 3628800.0));
        }}
        ...
        double exp_d(double x) {{
            if (x > MAX_EXP) return 1.0 / 0.0;     // +Inf
            if (x < MIN_EXP) return 0.0;           // underflow to 0
        
            // reduce x: x = k*ln2 + r, so exp(x) = 2^k * exp(r)
            double k_real = floor(x * INV_LN2 + 0.5);
            int k = int(k_real);
            double hi = k_real * LN2_HI;
            double lo = k_real * LN2_LO;
            double r = x - hi - lo;
        
            // compute exp(r) via polynomial
            double exp_r = exp_poly(r);
        
            // scale by 2^k
            return exp_r * exp2_d(k);
        }}

        // --- double-precision tanh(x) using exp_d ---
        ...
        double psiA(double x){{ return abs(x)<=1.0 ? 1.0 : 0.0; }}

        double psiB(double x){{
            return (x<-1.25)?0.0:
                   (x<-0.75)?0.5*(1.0+sin_d(2.0*3.14159265*x)):
                   (x<=0.75)?1.0:
                   (x<=1.25)?0.5*(1.0-sin_d(2.0*3.14159265*x)):
                              0.0;
        }}
...
float field(vec2 p){{
    double v     = 0.0;   // 加权后的数值累计
    double wsum  = 0.0;   // POU 权重和  Σ w_i
    ...
    for (int i = 0; i < NSUB; ++i){{
        ...
        for (int j = 0; j < NHIDDEN; ++j){{
            ...
            vec4 w_raw = texelFetch(weights_tex, ivec2(j, i), 0).rgba;  // weights RGBA32F
            dvec2 w = dvec2(w_raw.r + w_raw.g, w_raw.b + w_raw.a); // reconstruct two doubles

            vec2 b_raw = texelFetch(biases_tex, ivec2(j, i), 0).rg;   // biases RG32F
            double b = double(b_raw.r) + double(b_raw.g);          // reconstruct bias

            vec2 o_raw = texelFetch(outputW_tex, ivec2(j, i), 0).rg;  // output weights RG32F
            double o = double(o_raw.r) + double(o_raw.g);          // reconstruct output weight
            dvec2 s = dvec2(s.x, s.y);  // ensure s is double for dot product
            double a = act(dot(w, s) + b);   // 可切换激活函数
            sub += a * o;
        }}
    ...
    // -------- 归一化：Σ α_i = 1 --------
    return float((wsum > 0.0) ? (v / wsum) : 0.0);
}}
        ...
        if (sdf(p) > 0.0) {{
            ...
            float f = field(p);
            float t = clamp((f - vmin) / (vmax - vmin), 0.0, 1.0);
...
class RFMVisualizer3D(RFMVisualizer):
        ...
        void main(){{
            ...
            for(int i=0;i<MAX_STEPS;++i){{
                ...
                if(d<EPSILON){{
                    ...
                    float tt=clamp((f-vmin)/(vmax-vmin),0.0,1.0);
    ...
    def plot(self):
        ...
        self.ax.imshow(np_img, origin='lower')
        sm = plt.cm.ScalarMappable(cmap='viridis',
                                   norm=plt.Normalize(vmin=self.vmin, vmax=self.vmax))
        sm.set_array([])
        plt.colorbar(sm, ax=self.ax).set_label('Scalar Field Value')
