...
def load_config() -> Config:
    ...
    cfg = Config(
        ply_path=str(raw.get("ply_path", "bunny.ply")),
...
def main():
    ...
    # 读取 & 法向
    helper = PLYNormalHelper(cfg.ply_path, remeshing=cfg.remeshing)
    ...
    # 目录
    ckpt_dir = Path(cfg.out.checkpoints_dir) / tag
    snap_dir = Path(cfg.out.renders_dir) / tag
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    snap_dir.mkdir(parents=True, exist_ok=True)

    logs_dir = Path(cfg.logging.dir) / tag
    ...
    if RESUME and ckpt_last.exists():
        ...
        snap_dir.mkdir(parents=True, exist_ok=True)
        best_metric = float("inf")
        print(f"[Clean] checkpoints/ and renders/ are reset for tag={tag}.")
    ...
    try:
        ...
        with torch.no_grad():
            ...
            print("Mean |SDF(pts_n)|:", model(pts_n).abs().mean().item())
