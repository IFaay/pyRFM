...
def func_rhs(p: torch.Tensor) -> torch.Tensor:
    ...
    """
    return ((p[:, 0] * (p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) * (
            p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (p[:, 0] + 1)) * (
                     p[:, 0] ** 2 * (p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 * (
                     p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (
                     p[:, 0] + 1)) ** 2 + 4 * p[:, 1] ** 2 * (
                             p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 + p[:,
                                                                                                 2] ** 2) * (
                     p[:, 0] * (p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) * (
                     p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (
                     p[:, 0] + 1)) * torch.sin(p[:, 0]) + 2 * p[:, 1] * (
                             p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) * torch.sin(
                 p[:, 1] - p[:, 2]) * torch.cos(p[:, 0]) - p[:, 2] * torch.sin(p[:, 1] - p[:, 2]) * torch.cos(
                 p[:, 0])) - 2 * p[:, 1] * (p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) * (
                     p[:, 0] ** 2 * (p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 * (
                     p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (
                     p[:, 0] + 1)) ** 2 + 4 * p[:, 1] ** 2 * (
                             p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 + p[:,
                     ...
                     -p[:, 0] * (p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) * (
                     p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (
                     p[:, 0] + 1)) * torch.sin(p[:, 1] - p[:, 2]) * torch.cos(p[:, 0]) + 2 * p[:, 1] * (
                             p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) * (
                 ...
                 p[:, 0])) + 2 * (torch.sin(p[:, 1] - p[:, 2]) ** 2 - torch.cos(p[:, 1] - p[:, 2])) * (
                     ...
                     p[:, 0] + 1)) ** 2 + 4 * p[:, 1] ** 2 * (
                             p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 + p[:,
                                                                                                 2] ** 2) ** 2 * torch.sin(
        p[:, 0]) - (p[:, 0] ** 2 * (p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 * (
            p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (p[:, 0] + 1)) ** 2 + 4 * p[:,
                                                                                                              1] ** 2 * (
                            p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 + p[:,
                                                                                                2] ** 2) ** 2 * torch.sin(
        p[:, 0]) + (p[:, 0] * (p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) * (
            p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (p[:, 0] + 1)) * torch.cos(
        p[:, 0]) - 2 * p[:, 1] * (p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) * torch.sin(
        p[:, 0]) * torch.sin(p[:, 1] - p[:, 2]) + p[:, 2] * torch.sin(p[:, 0]) * torch.sin(p[:, 1] - p[:, 2])) * (
                     p[:, 0] ** 2 * (p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 * (
                     p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (
                     p[:, 0] + 1)) ** 2 * (p[:, 0] ** 2 * (
                     p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (
                     p[:, 0] + 1)) ** 2 + 4 * p[:, 1] ** 2 + 2 * (
                                                   p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:,
                                                                                                  1] ** 2) * (
                                                   p[:, 0] ** 2 + 2 * p[:, 0] * (p[:, 0] - 1) + 2 * p[:,
                                                                                                    0] * (
                                                           p[:, 0] + 1) + (p[:, 0] - 1) * (
                                                           p[:, 0] + 1))) - p[:, 0] ** 2 * (
                             p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 * (
                             p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (
                             p[:, 0] + 1)) ** 2 - p[:, 0] ** 2 * (
                             p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (
                             p[:, 0] + 1)) ** 2 * (p[:, 0] ** 2 * (
                     p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 * (
                                                           ...
                                                           p[:, 0] + 1) + p[:, 1] ** 2) ** 2 + p[:,
                                                                                               2] ** 2) + 4 * p[
                                                                                                              ...
                                                                                                              1] ** 2 * (
                             p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 * (
                             2 * p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 0] ** 2 * (
                             p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (
                             p[:, 0] + 1)) ** 2 + 6 * p[:, 1] ** 2) - 4 * p[:, 1] ** 2 * (
                             p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 - 4 * p[:,
                                                                                                     1] ** 2 * (
                             p[:, 0] ** 2 * (
                             p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 * (
                                     p[:, 0] * (p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (
                                     p[:, 0] - 1) * (p[:, 0] + 1)) ** 2 + 4 * p[:, 1] ** 2 * (
                                     p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 + p[
                                                                                                         :,
                                                                                                         2] ** 2) - 2 * (
                             p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) * (p[:, 0] ** 2 * (
                     p[:, 0] ** 2 * (p[:, 0] - 1) * (p[:, 0] + 1) + p[:, 1] ** 2) ** 2 * (p[:, 0] * (
                     p[:, 0] - 1) + p[:, 0] * (p[:, 0] + 1) + 2 * (p[:, 0] - 1) * (p[:, 0] + 1)) ** 2 + 4 * p[:,
                                                                                                            1] ** 2 * (
                                                                                                     p[:,
                                                                                                     0] ** 2 * (
                                                                                                             p[
...
if __name__ == '__main__':
    ...
    x_on = torch.tensor([0.0, 0.0, 0.1]).view(1, -1)
